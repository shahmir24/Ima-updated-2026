/**
 * Body Double "Break it down" — the wire contract.
 *
 * This is not an AI framework. It describes exactly one exchange: the user is
 * stuck on a task and asks for a smaller, concrete next action. Nothing else
 * belongs here — no rationale, no confidence, no estimated minutes, no mood,
 * no conversation history, no user id, no session or journal data. Those are
 * either invented, expensive, or somebody else's concern.
 *
 * There are two copies of this module: this one, built by Vite for the browser,
 * and supabase/functions/_shared/breakdown-contract.ts, run by Deno at the
 * edge. The two runtimes cannot share a module, so the file is duplicated on
 * purpose and the two are held together by a parity test rather than by an
 * import. Both are dependency-free so that "equivalent" can be proved by
 * running the same inputs through both, which two different validation engines
 * could never demonstrate.
 *
 * Validate on both sides. The edge validates so malformed model output never
 * reaches a client; the client validates so a stale or compromised function
 * cannot push arbitrary text into the UI.
 */
